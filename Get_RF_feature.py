import sys
import os
def get_RF_features(Dir,whole_mature,whole_ID_mature,whole_ID, whole_miBase,\
                  whole_RF,whole_miBase_RF,whole_ID_miBase, whole_ID_RF,whole_ID_miBase_,fw_name):
    files=os.listdir(Dir)
    fw=open("RF_"+fw_name+"_tmp","w+")
    ...
    for file in files:
        ...
        none_RF_family_number=0
        reads_sum_in_none_RF=0
        for line in fr.readlines()[1:]:
            s = line.strip().split("\t")
            if s[5]=="No RF_family":
                none_RF_family_number+=1
                reads_sum_in_none_RF+=float(s[2])
            reads_sum_in_file=reads_sum_in_file+float(s[2])
            Data_list.append(s) # the Data_list is the [[] ,[] .[]]
        fr.close()
        Data_list.sort(key=lambda x:(x[5],x[0]))#sort by mature and then ID
        fr.close()
        #now start to iter the list
        pre_family = Data_list[0][5]
        ...
        feature_line=str(feature_family_number)+"\t"+str(feature_everyline_avg_in_block)+"\t"+\
            str(float(none_RF_family_number)/len(Data_list))+"\t"+str(float(reads_sum_in_none_RF)/reads_sum_in_file)
    ...
    fw.close()
    fr = open("RF_"+fw_name+"_tmp","r")
    fw = open("RF_"+fw_name,"w+")
    ...
    fr.close()
    fw.close()
    os.remove("RF_"+fw_name+"_tmp")
#        print "the RF feature number="+str(len(whole_RF))+"+"+str(len(whole_ID_RF))+"+"+\
#               str(len(whole_RF))+"+4 ="+str(len(whole_RF)+len(whole_ID_RF)+len(whole_RF)+4)
#        print "the real featuer number is : "+str(len(feature_line.strip().split("\t")))
#        if len(whole_RF)+len(whole_ID_RF)+len(whole_RF)+4!=len(feature_line.strip().split("\t")):
#            print "the Get_RF is no equal"
#            exit()
    #end